...
export class DrizzleGeneratedExecutionStore implements GeneratedExecutionStore {
  async load(taskId: number, planId: string): Promise<GeneratedExecutionCheckpoint | undefined> {
    ...
    // Durable owner check: the checkpoint's embedded userId must match the task owner.
    if (task && parsed.data.userId !== task.userId) {
      throw new Error("Generated execution does not belong to the task owner");
    }
    return parsed.data;
  }
  ...
  async save(checkpoint: GeneratedExecutionCheckpoint): Promise<void> {
    ...
    // Durable owner check: only the task's owner may persist execution state for it.
    if (task.userId !== parsed.userId) throw new Error("Generated execution does not belong to the task owner");
    const context = task.context as Record<string, unknown> | null;
    const executions = context?.generatedExecutions as Record<string, unknown> | undefined;
  ...
  async requestApproval(request: ApprovalRequest): Promise<GeneratedApprovalTicket> {
    // Durable owner check: only the task's owner may request an approval for it.
    ...
    if (task.userId !== request.userId) throw new Error("Generated approval does not belong to the task owner");
