...
export class DrizzleDNARepository implements DNARepository {
  ...
  async saveCandidate(candidate: GeneCandidate): Promise<void> {
    ...
    await db.insert(dnaCandidates).values(values).onConflictDoUpdate({
      target: dnaCandidates.id,
      set: {
  ...
  async saveVersion(version: GeneVersion): Promise<void> {
    ...
    await db.insert(dnaVersions).values(values).onConflictDoUpdate({
      target: dnaVersions.id,
      set: {
        status: values.status,
  ...
  async saveSnapshot(snapshot: GenomeSnapshot): Promise<void> {
    ...
    }).onConflictDoUpdate({ target: dnaGenomeSnapshots.id, set: { geneVersionIds: snapshot.geneVersionIds } });
  }
