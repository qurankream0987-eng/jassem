...
} from "../runtime/block3/provider-output-boundary";
...
function buildResponsePrompt(ctx: ResponseContext): string {
  ...
  // P0 (Block 3 §8): task outputs contain third-party provider content. They
  // cross into privileged model context ONLY through the untrusted-output
  // boundary — projected, size-limited, and enveloped as inert data.
  const outputText = ctx.taskOutputs && Object.keys(ctx.taskOutputs).length > 0
    ? safeProviderOutputForModel(ctx.taskOutputs, { source: "task-execution-outputs" })
    : "No outputs yet";
    ...
    if (ctx.executionError) {
      ...
      if (ctx.taskOutputs && Object.keys(ctx.taskOutputs).length > 0) {
        const summary = Object.entries(ctx.taskOutputs)
          .map(([k, v]) => `${k}: ${summarizeProviderValue(v)}`)
          .join("; ");
