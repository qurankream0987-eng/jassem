...
export function analyzeComplexity(request: LLMRequest): ComplexityAnalysis {
  ...
  const isPremium = /legal|law|contract|financial audit|compliance|regulatory|
    قانون|عقد|مالية|تدقيق|امتثال/i.test(combined);

  const isComplex = /negotiate|compare multiple|analyze|optimize|reason|
    فاوض|قارن|حلل|حساب/i.test(combined) ||
    prompt.length > 500;
