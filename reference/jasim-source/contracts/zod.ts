...
const taskStatusSchema = z.nativeEnum(TASK_STATUSES);
const taskPrioritySchema = z.nativeEnum(TASK_PRIORITIES);
const capabilityRiskLevelSchema = z.nativeEnum(CAPABILITY_RISK_LEVELS);
const capabilitySideEffectSchema = z.nativeEnum(CAPABILITY_SIDE_EFFECTS);
const agentStatusSchema = z.nativeEnum(AGENT_STATUSES);
const entityTypeSchema = z.nativeEnum(ENTITY_TYPES);
const commerceActionSchema = z.nativeEnum(COMMERCE_ACTIONS);
const memoryScopeSchema = z.nativeEnum(MEMORY_SCOPES);
const memoryCategorySchema = z.nativeEnum(MEMORY_CATEGORIES);
const eventTypeSchema = z.nativeEnum(EVENT_TYPES);
const bubbleTypeSchema = z.nativeEnum(BUBBLE_TYPES);
const dnaPrimitiveSchema = z.nativeEnum(DNA_PRIMITIVES);
const messageRoleSchema = z.nativeEnum(MESSAGE_ROLES);
const executionStatusSchema = z.nativeEnum(EXECUTION_STATUSES);

// ═══════════════════════════════════════════════════════════════════════════════
// Base / Shared Schemas
// ═══════════════════════════════════════════════════════════════════════════════

export const metadataSchema: z.ZodType = z.record(z.unknown());
